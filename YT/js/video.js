...
function getVideoCode(link) {
    ...
    //Code was copied from a google image search
    const googleImageSearchPrefix = "i.ytimg.com%2Fvi%2F"
    const googlePrefixLen = googleImageSearchPrefix.length

    if (!(link.match(googleImageSearchPrefix) == null)) {
        const startIdx = link.search(googleImageSearchPrefix)

        link = link.slice(startIdx + googlePrefixLen, startIdx + googlePrefixLen+ ytCodeSize)
